...
import { RotateCcw, Download, AlertCircle, Eye, CheckCircle, Clock, Mail } from 'lucide-react';
...
export const EmailMonitoring = () => {
  ...
  return (
    <div className="flex flex-col gap-6 relative">
      ...
      {!mailboxLoading && !isConnected && (
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-2xl p-6 md:p-8 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            ...
            <h2 className="text-xl md:text-2xl font-bold">Connect Gmail to monitor emails</h2>
            <p className="text-sm text-blue-100 leading-relaxed">
              Email monitoring is currently paused. Connect your Gmail account to start tracking and analyzing incoming emails.
            </p>
          </div>
          <a
            href="/settings"
      ...
      {/* Live Status Indicator - Only show when connected */}
      {isConnected && (
      ...
      {/* Dynamic KPI Cards - Only show when connected */}
      {isConnected && (
      ...
      {!mailboxLoading && !isConnected && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {tabs.map(tab => (
            <div key={tab.id} className="flat-card flex flex-col items-center justify-center p-4">
              <span className="text-on-surface-variant text-sm font-medium uppercase tracking-wider">{tab.label}</span>
              <span className="text-3xl font-bold text-gray-300 mt-1">0</span>
            </div>
          ))}
        </div>
      )}

      {/* Filters and controls - Only show when connected */}
      {isConnected && (
        <>
          ...
          {(error || actionError) && (
            <div className="bg-surface-container p-4 rounded-xl text-body-md flex items-center gap-3 border border-outline-variant">
              <AlertCircle size={18} className="text-on-surface-variant shrink-0" />
              <span className="text-on-surface-variant">{error || actionError}</span>
              {error && <button onClick={refresh} className="ml-auto text-primary text-sm font-semibold hover:underline whitespace-nowrap">Retry</button>}
            </div>
          )}
